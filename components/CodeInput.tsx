...
const CodeInput: React.FC<CodeInputProps> = ({ onSearch, isLoading, selectedBrand, selectedModel, lang, onImageAnalysis, setIsLoading, onReset }) => {
  ...
  const handleImageAnalyze = async (imageData: string) => {
    ...
    setIsLoading(true);
    setProgress(0);
    setScanText('INITIALIZING SECURITY...');
    soundEngine.playBoot();
    
    const simulatedCodes = [
      "SCANNING IMAGE PATTERN...", "CHECKING FOR DASHBOARD...", "VALIDATING DIAGNOSTIC DATA...",
      "IDENTIFYING COMPONENT...", "DETECTING PID...", "ANALYZING FMI...",
      "STANDARD 70 627 LOADED", "EXTRACTING DATA..."
    ];
    ...
    try {
      ...
      if (error.message === "INVALID_IMAGE_CONTENT") {
        alert(lang === 'ar' 
            ? 'صورة غير صالحة: يرجى مسح كود عطل، طبلون، أو قطعة غيار (شاحنة).' 
            : 'Invalid Image: Please scan a Fault Code, Dashboard, or Truck Spare Part.');
                ...
                {(isStreaming || isLoading) && (
                  <div className="absolute inset-0 z-30 pointer-events-none">
                      ...
                      <div className="absolute top-4 left-4 flex flex-col gap-1">
                        <span className="text-[8px] bg-red-600 text-white px-1 font-bold">REC</span>
                        <span className="text-[8px] font-mono text-zinc-400">{selectedBrand.toUpperCase()} DIAG</span>
                      </div>

                      <div className="absolute bottom-24 w-full text-center">
                        <div className="inline-flex items-center gap-2 bg-black/60 px-3 py-1 rounded-full backdrop-blur-sm border border-white/10">
                           <AlertTriangle className="w-3 h-3 text-yellow-500" />
                           <span className="text-[10px] text-yellow-200 font-bold">
                             {lang === 'ar' ? 'امسح كود، طبلون، أو قطعة غيار' : 'Scan Code, Dashboard, or Spare Part'}
                           </span>
                        </div>
                      </div>
                  </div>
